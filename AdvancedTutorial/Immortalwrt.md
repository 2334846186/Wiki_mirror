...
4月8日以后的 R3G固件均会自动挂载硬盘，如未挂载，登录后台-系统-挂载点 点击挂载已连接设备
...
名称可随意设置（最好不要是中文），路径中的`/mnt/sda1`为磁盘挂载的路径，匿名建议打开强制 Root选项

如果想关闭匿名访问并使用root登录，需要在网络存储-网络共享-编辑模板

在该行代码前添加一个`#`号，然后保存并应用

![](./Image/1-10.png)

接着使用 ttyd 或者 ssh 输入以下两行代码
...
接着会要去你为 samba 的 root 账户设置一个密码，输入两边回车，然后重启路由器即可
